...
def test_get_s3_client_success(monkeypatch):
    ...
    monkeypatch.setattr(s3_mod, "get_config", lambda *_: MOCK_CONFIG)
...
def test_get_s3_client_no_config(monkeypatch):
    monkeypatch.setattr(s3_mod, "get_config", lambda *_: None)
...
def test_get_s3_client_no_url(monkeypatch):
    ...
    cfg.pop("url")
    monkeypatch.setattr(s3_mod, "get_config", lambda *_: cfg)
    with pytest.raises(ValueError):
        s3_mod.get_s3_client()
...
def test_is_s3_service_available_false(monkeypatch):
    monkeypatch.setattr(s3_mod, "get_s3_client", lambda: (_ for _ in ()).throw(Exception("fail")))
...
def test_create_s3_bucket_success(monkeypatch, mock_s3_client):
    monkeypatch.setattr(s3_mod, "bucket_exists", lambda b: False)
    monkeypatch.setattr(s3_mod, "get_s3_client", lambda: mock_s3_client)
    assert s3_mod.create_s3_bucket("bucket1")
...
def test_delete_s3_bucket_not_exists(monkeypatch):
    ...
    monkeypatch.setattr(s3_mod, "get_s3_client", mock_get_s3_client)
    s3_mod.delete_s3_bucket("bucket1")
...
def test_list_files_for_dates(monkeypatch):
    ...
    monkeypatch.setattr(s3_mod, "make_date_prefix", lambda date: f"prefix/{date}")
    # Patch list_files to return predictable files
    monkeypatch.setattr(s3_mod, "list_files", lambda prefix, bucket: [f"{prefix}/file1.txt", f"{prefix}/file2.txt"])
