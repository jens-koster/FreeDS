...
import tfds.spark.spark as spark_mod
...
def patch_get_config():
    with mock.patch("tfds.spark.spark.get_config", return_value=MOCK_CONFIG):
        yield
...
def patch_pyspark_and_delta(monkeypatch, mock_spark_conf, mock_spark_builder):
    ...
    monkeypatch.setattr("tfds.spark.spark.configure_spark_with_delta_pip", delta_mock)
    return delta_mock
